...
import { UnitCalculation } from "./types";
...
export function normalize(unit: UnitCalculation, statType: Status): number {
  switch (statType) {
    case Status.Crit:
      return _actualStat(
        StatusCapMap[Status.Crit],
        unit.finalStats[statType]! * 10
      );
    case Status.Acc:
      return _actualStat(
        StatusCapMap[Status.Acc],
        unit.finalStats[statType]! * 10
      );
    case Status.CCAcc:
      return _actualStat(
        StatusCapMap[Status.CCAcc],
        unit.finalStats[statType]! * 10
      );
    case Status.CritResistance:
      return _actualStat(
        StatusCapMap[Status.CritResistance],
        unit.finalStats[statType]! * 10
      );
    case Status.CCResistance:
      return _actualStat(
        StatusCapMap[Status.CCResistance],
        unit.finalStats[statType]! * 10
      );
    case Status.PDodge:
    case Status.MDodge:
    ...
    case Status.MTough:
      return _actualStat(
        StatusCapMap[Status.Pen],
        unit.finalStats[statType]! * 10
      );
    case Status.AtkSpd:
      return _actualStat(
        StatusCapMap[Status.AtkSpd],
        unit.finalStats[statType]! * 10
      );
    case Status.PBlockDef:
    case Status.MBlockDef:
      return _actualStat(
        StatusCapMap[Status.BlockDef],
        unit.finalStats[statType]! * 10
      );
    case Status.ManaPerAttack:
      return _actualStat(
        StatusCapMap[Status.ManaPerAttack],
        unit.finalStats[statType]! * 10
      );
    case Status.Atk:
      return _actualStat(
